...
These days, the criminal justice system relies on all sorts of algorithms to plan patrol routes, manage resources, and try to prevent crime~\cite{perry2013predictive}. Machine learning, in particular, can spot patterns and make sharp predictions about where crime might happen next, using old crime data, socioeconomic info, and trends over time~\cite{lum2016predict, richardson2019dirty}. But here's the problem: the data itself is already biased. Past crime records reflect not just what happened, but also the way police operate, how different communities report crimes, and deep-seated social inequalities.

If you train machine learning models on this kind of data, without considering fairness, you end up making things worse~\cite{angwin2016machine, barocas2016big}. More police get sent to neighborhoods that already feel over-policed, while vulnerable groups still don't get the help they need. Most crime prediction models focus only on accuracy---looking at numbers like Mean Absolute Error, R-squared, or RMSE. That's it. They don't think about fairness at all. So, even if the overall numbers look good, the models might be way off for certain protected groups.

This shows up in a few ways:
\begin{itemize}
    \item The models can miss or overestimate crimes that affect women and children.
    \item Some groups get much less accurate predictions than others.
    \item If resources get distributed based on these skewed predictions, the same old problems just keep repeating.
    \item Modeling groups separately doesn't really show how crime patterns change for the most vulnerable people.
\end{itemize}

What we really need is a crime prediction system that's not just accurate, but fair---one that works well for every protected group.

\subsection{Research Objectives}

Here's what we're aiming for with this study:
\begin{enumerate}
    \item Build a deep learning model that can juggle multiple tasks and clearly show crime patterns for four protected groups: SC, ST, Women, and Children.
    \item Add fairness constraints during training to keep prediction gaps between vulnerable populations as small as possible.
    \item Test these fairness standards against the usual accuracy benchmarks, using real crime data.
    \item Compare our proposed FC-MT-LSTM model to six top baseline models.
    \item Show how this approach could actually help split up police resources more fairly in the real world.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{New Architecture}: We introduce FC-MT-LSTM. It combines spatial CNNs, attention-based LSTMs for time, shared encoding, and group-specific decoders with fair loss functions.
    \item \textbf{Extensive Benchmarking}: We test against six different baselines, including statistical models (SARIMA, Prophet), ensemble methods (Random Forest, XGBoost), and deep learning setups (CNN-LSTM, Transformer).
    \item \textbf{Full Fairness Evaluation}: We develop metrics that directly measure prediction differences between SC, ST, Women, and Children---so we can see where the models fall short for each group.
    \item \textbf{Real-World Dataset}: We pull from six years of expanded crime data from India's NCRB, covering more than 700 districts and including protected category labels.
    \item \textbf{Practical Framework}: All our code is available in both PyTorch and TensorFlow.
\end{itemize}
...
This section looks at crime prediction models, algorithmic fairness in criminal justice, multi-task learning, and fairness-aware methods. Table~\ref{tab:literature_survey} lays out a detailed comparison of existing approaches.

\subsection{Crime Prediction Models}

Crime forecasting has come a long way, moving from old-school statistical methods to cutting-edge deep learning. Early research leaned on time-series models like ARIMA to track crime patterns over time~\cite{box2015time}. Then, things got more sophisticated---geographically weighted regression brought spatial context into the picture~\cite{anselin2000spatial}. Machine learning models like Random Forest and XGBoost pushed accuracy further by capturing the messy, non-linear relationships between economic or social factors and crime~\cite{breiman2001random, chen2016xgboost}. Facebook's Prophet also works well when the data has strong, repeating trends~\cite{taylor2018forecasting}.

Deep learning models have set new records lately. CNNs, for example, dig into the spatial features in crime heatmaps~\cite{huang2018deepcrime}. Attention mechanisms highlight important time periods, and LSTMs capture how crime evolves over time~\cite{hochreiter1997long}. Even Transformer models, which started in language processing, are getting adapted for crime prediction and showing real promise~\cite{vaswani2017attention}. Still, almost all of these studies zero in on accuracy and brush aside issues of fairness between different groups.

\subsection{Algorithmic Fairness in Criminal Justice}

After high-profile cases like the COMPAS recidivism prediction tool revealed major racial biases~\cite{angwin2016machine, dressel2018accuracy}, questions about fairness in criminal justice algorithms exploded. Researchers have come up with different ways to measure fairness. Demographic parity says every group should get the same share of positive predictions. Equalized odds goes a step further, insisting that true and false positive rates should match across groups. Predictive parity wants the chance of a correct prediction (once the model says ``yes'') to be equal everywhere~\cite{verma2018fairness}. But here's the catch: mathematically, you can't satisfy all these fairness rules at once---unless the data is trivial~\cite{kleinberg2016inherent}.

Fairness interventions fall into three buckets. Pre-training methods tweak the data before the model sees it. In-processing methods add fairness constraints while the model learns. Post-training approaches adjust model outputs after training~\cite{hardt2016equality}. Our FC-MT-LSTM uses an in-processing method, directly penalizing unfairness in the loss function.

\subsection{Multi-Task Learning for Crime Prediction}

Multi-task learning (MTL) lets models share knowledge between related prediction tasks, which helps them generalize better and learn from fewer examples~\cite{caruana1997multitask}. In crime forecasting, MTL can model several crime types or different locations at the same time~\cite{zhang2021survey}.

We take this further. Our FC-MT-LSTM doesn't define tasks by crime type; instead, it focuses on protected groups. This group-centric approach lets us target fairness for vulnerable populations directly. The model's shared encoder learns general crime patterns, while the group-specific decoders pick up what matters for SC, ST, Women, and Children.

\subsection{Novelty of Proposed Work}

FC-MT-LSTM stands out in three ways: (1) Instead of classic MTL that focuses on crime types~\cite{patel2023multi}, our model optimizes directly for fairness among protected groups. (2) Our in-processing strategy penalizes unfairness across time and demographic groups using a pairwise MAE penalty---while past work like Wu and Frias-Martinez~\cite{wu2024improving} focused on spatial fairness, we tackle temporal fairness. (3) We support both PyTorch and TensorFlow, and benchmark against six different baselines---rarely done in this field~\cite{mohammadi2023survey, wang2023fairness}.
...
The Spatial CNN captures district-level crime characteristics through convolutional layers:
\begin{equation}
...
The temporal module employs bidirectional LSTM with additive attention:
\begin{equation}
...
The attention mechanism identifies important timesteps:
\begin{equation}
...
The shared encoder fuses spatial and temporal representations:
\begin{equation}
...
Four parallel decoders generate predictions for each protected group:
\begin{equation}
...
The total loss combines prediction accuracy and fairness:
\begin{equation}
...
\midrule
\textbf{FC-MT-LSTM (Ours)} & 6.54 & 16.05 & 0.9922 & 12.61 & \textbf{1.99} & 8.50 \\
\bottomrule
\multicolumn{7}{l}{\footnotesize *CNN-LSTM and Transformer did not produce per-group breakdowns.} \\
\multicolumn{7}{l}{\footnotesize Bold indicates best performance.}
\end{tabular}
\end{table*}

\textbf{Key Observations}:
\begin{itemize}
    \item \textbf{Ensemble methods} (Random Forest, XGBoost) achieve excellent accuracy (MAE $<$ 2.5, R$^2$ $>$ 0.999) but exhibit severe fairness violations (Fairness Ratio: 3.73--4.74).
    \item \textbf{Statistical methods} (SARIMA, Prophet) demonstrate poor accuracy (MAE 135--167) with moderate fairness ratios (1.17--1.22).
    \item \textbf{\modelname{}} achieves balanced trade-off: competitive accuracy (MAE: 6.54, R$^2$: 0.9922) with substantially improved fairness (Ratio: 1.99).
\end{itemize}

The fairness ratio of 1.99 indicates that the worst-performing group experiences MAE only 99\% higher than the best-performing group---a substantial improvement over ensemble methods' 273--374\% disparities.
